...
## Real Entrypoints
- Use `python3 pseudodata.py` for toy studies, `python3 signal.py` for standalone signal fits, `python3 resonant_background.py` for standalone resonant-background fits, and `python3 realdata.py` for the real-data dimuon-plus-sideband workflow.
...
## Workflow Gotchas
- `pseudodata.py` and `realdata.py` delete all files under `plots/` except `.gitkeep` at startup. `signal.py` only clears `plots/signal_fit/`, and `resonant_background.py` only clears `plots/resonant_background/`.
- `run_fit_2d_data()` still deletes repo-root `*.json` caches unless `--use-cache` is passed. Use `python3 realdata.py --use-cache` for iterative work if you want to preserve `upsilon_model_params.json`.
...
- `realdata.py` no longer runs signal modeling or resonant-background modeling. Run those explicitly via `python3 signal.py` and `python3 resonant_background.py --use-cache` when needed.
- Do not run `python3 realdata.py` unless the user explicitly asks for it; it is intentionally treated as a long-running manual workflow.
...
## Verification
- The smallest verified syntax check is `python3 -m compileall pseudodata.py realdata.py signal.py resonant_background.py statistical_test_fit`.
